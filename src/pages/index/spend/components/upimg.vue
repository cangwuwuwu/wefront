<template>
    <div class="first step">
        ...
        <div class="demo-upload-list" v-for="(item, index) in uploadList" :key="index">
            ...
            <template v-else>
                <i-circle v-if="item.showProgress" :size="80" :percent="item.percentage"
                          :stroke-color="item.percentage === 100 ? '#5cb85c': '#2db7f5'" :stroke-width="7">
                    <Icon v-if="item.percentage === 100" type="ios-checkmark" size="60" style="color:#5cb85c"></Icon>
                    <span v-else class="demo-Circle-inner"
                          style="font-size:24px">{{ parseInt(item.percentage) }}%</span>
                </i-circle>
