...
export default {
    ...
    data() {
        const validatePassCheck = (rule, value, callback) => {
            if (value !== this.formItem.stuPassword) {
        ...
        const IdUsedCheck = (rule, value, callback) => {
            this.checkId(value).then(result => {
                if (result === 1) callback(new Error('该学号已被注册')) 
                else callback();
            })
        };
        const NameUsedCheck = (rule, value, callback) => {
            this.checkName(value).then(result => {
                if (result === 1) callback(new Error('该用户名已被注册')) 
                else callback();
            })
        };
        return {
    ...
    methods: {
        ...
        sendMail() {
            ...
            params.append('email', this.formItem.stuEmail)
            axios
            .post('/api/sendmail/sign', params)
            .then(function (data_mail) {
                // 发送成功禁用按钮/倒计时/解禁按钮
        ...
        Submit (name) {
            if (this.formItem.stuId === '' || this.formItem.stuUsername === '') return;
            let _self = this;
            let fields = ['stuPhone', 'stuPassword', 'stuRePassword', 'stuInfo', 'stuCode', 'stuEmail'];
            Promise.all(fields.map(item => {
                ...
                if (data.join(',').length === 5) {
                    ...
                    formData.append('stuUsername', this.formItem.stuUsername)
                    ...
                    formData.append('stuCode', this.formItem.stuCode)
                    formData.append('stuBirthday', 
                    this.formItem.stuBirthday === '' ? this.formItem.stuBirthday 
                    : formatDate(new Date(this.formItem.stuBirthday), 'yyyy-MM-dd'))
                    formData.append('stuArea', this.formItem.stuArea)
                    formData.append('stuInfo', this.formItem.stuInfo)
                    formData.append('stuPassword', this.formItem.stuPassword)
                    formData.append('stuRePassword', this.formItem.stuRePassword)
                    axios
        ...
        async checkId(id) {
            var res =  await axios.get('/api/stu/id/'+ id)
            if (res.data !== '') return 1
            else return 0
        },
        async checkName(name) {
            var res =  await axios.get('/api/stu/username/'+ name)
            if (res.data !== '') return 1
            else return 0
        },
        Reset (name) {
